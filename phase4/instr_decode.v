...
module instr_decode (
    ...
    assign num_write = (reg_dst == 2'b00) ? rt : // I-type指令写回rt寄存器
                       (reg_dst == 2'b01) ? instruction[15:11] : // R-type指令写回rd寄存器
                       (reg_dst == 2'b10) ? 5'b11111 : 5'b00001;// jal指令写回$ra寄存器
    ...
    always @(*) begin
    	imm32 = {{16{imm16[15]}}, imm16};  // 先给默认值（符号扩展）
        case (opcode)
            6'b001100,  // andi
            6'b001101:  // ori
